...
abstract class Grid extends \Nette\Application\UI\Control
{
	...
	/** @var array */
	protected $perPageValues = array(30 => 30, 50 => 50, 100 => 100, 100000=>100000);
	...
	protected function attached($presenter)
	{
		...
		$this->messageNoRecords = "No Records";
	...
	protected function createComponentGridForm()
	{
		...
		$form[$this->name]['rowForm']->addSubmit("send","Odeslat");
		...
		$form[$this->name]['perPage']->addSelect("perPage","Záznamů na stránku", $this->perPageValues)
			->getControlPrototype()
				...
				if(isset($this['gridForm'][$this->name]['rowForm'][$name])){
					...
					if($input instanceof \Nette\Forms\Controls\SelectBox){
						...
						if(in_array($row[$name], $items)){
							$val=array_search($row[$name], $items);
							$row->update(array($name=>$val));
						}
					}
				}
			}
